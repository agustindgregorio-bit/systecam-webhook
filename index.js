...
// Send notification to Karen's WhatsApp directly
var KAREN_PHONE = "5491134298519";
...
function sendKarenNotification(phone, userText, botResponse, state, clientName) {
  try {
    ...
    var msg = "*🔔 Nueva consulta de cliente*\n\n";
    msg += "*📞 Tel:* " + phone + "\n";
    msg += "*👤 Cliente:* " + name + "\n";
    msg += "*💬 Escribio:* " + (userText || "").substring(0, 200) + "\n";
    msg += "*🤖 Respuesta:* " + (botResponse || "").substring(0, 200) + "\n";
    msg += "*📍 Estado:* " + (state || "START");
    sendWhatsApp(KAREN_PHONE, msg).catch(function(err) {
      console.error("Error sending Karen notification:", err.message);
    });
    console.log("Notification sent to Karen");
  } catch (err) {
    console.error("Error sendKarenNotification:", err.message);
  }
}
